...
async def search_user_in_community(client, message):
    try:
        await client.get_chat_member('@KOT_BOTS', message.chat.id)
        await client.get_chat_member('@KOT_REPORS', message.chat.id)
    except UserNotParticipant:
        await message.reply_text(not_joined_community, parse_mode = 'html',reply_markup=InlineKeyboardMarkup([
        [InlineKeyboardButton('Join our Channel.',url = 'https://t.me/KOT_BOTS')],
        [InlineKeyboardButton('Join our Group.',url = 'https://t.me/KOT_REPORS')]
        ]))
        return
